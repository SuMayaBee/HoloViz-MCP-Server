...
logger = logging.getLogger("holoviz_mcp_server")


def _default_user_dir() -> Path:
    return Path(os.getenv("HOLOVIZ_MCP_SERVER_USER_DIR", "~/.holoviz-mcp-server")).expanduser()
...
def _resolve_external_url(port: int) -> str:
    ...
    1. HOLOVIZ_MCP_SERVER_EXTERNAL_URL — explicit override
    2. JUPYTERHUB_HOST + JUPYTERHUB_SERVICE_PREFIX — JupyterHub
    3. CODESPACE_NAME — GitHub Codespaces
    4. "" — local fallback
    """
    if explicit := os.getenv("HOLOVIZ_MCP_SERVER_EXTERNAL_URL", ""):
...
def get_config() -> Config:
    ...
    global _config
    if _config is None:
        port = int(os.getenv("HOLOVIZ_MCP_SERVER_PORT", "5077"))
        _config = Config(
            port=port,
            host=os.getenv("HOLOVIZ_MCP_SERVER_HOST", "127.0.0.1"),
            max_restarts=int(os.getenv("HOLOVIZ_MCP_SERVER_MAX_RESTARTS", "3")),
            db_path=Path(
                os.getenv("HOLOVIZ_MCP_SERVER_DB_PATH", str(_default_user_dir() / "snippets" / "snippets.db"))
            ),
