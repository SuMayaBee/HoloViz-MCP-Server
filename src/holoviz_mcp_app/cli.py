...
if sys.platform == "win32":
    from holoviz_mcp_app.display.utils import prepend_env_dll_paths

    prepend_env_dll_paths(os.environ)

import typer

from holoviz_mcp_app import __version__
...
def version_callback(value: bool) -> None:
    if value:
        typer.echo(f"holoviz-mcp-app {__version__}")
        raise typer.Exit()
...
@app.command()
def serve(
    port: int = typer.Option(5077, "--port", "-p", help="Port for Panel server.", envvar="HOLOVIZ_MCP_APP_PORT"),
    host: str = typer.Option("localhost", "--host", "-H", help="Host address.", envvar="HOLOVIZ_MCP_APP_HOST"),
    db_path: str | None = typer.Option(None, "--db-path", help="SQLite database path.", envvar="HOLOVIZ_MCP_APP_DB_PATH"),
    ...
    os.environ["HOLOVIZ_MCP_APP_PORT"] = str(port)
    os.environ["HOLOVIZ_MCP_APP_HOST"] = host
    if db_path:
        os.environ["HOLOVIZ_MCP_APP_DB_PATH"] = db_path

    from holoviz_mcp_app.config import reset_config

    reset_config()

    from holoviz_mcp_app.display.app import main as app_main
...
def mcp(
    ...
    from holoviz_mcp_app.server.compose import get_composed_server
...
@app.command()
def status(
    port: int = typer.Option(5077, "--port", "-p", envvar="HOLOVIZ_MCP_APP_PORT"),
    host: str = typer.Option("localhost", "--host", "-H", envvar="HOLOVIZ_MCP_APP_HOST"),
) -> None:
